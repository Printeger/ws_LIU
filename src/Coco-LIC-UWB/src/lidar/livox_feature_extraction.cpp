...
bool LivoxFeatureExtraction::ParsePointCloudR3LIVE(
  ...
  for (uint i = 1; i < plsize; i++) {
    if ((lidar_msg->points[i].line < n_scan) &&
        (!IS_VALID(lidar_msg->points[i].x)) &&
        (!IS_VALID(lidar_msg->points[i].y)) &&
        (!IS_VALID(lidar_msg->points[i].z)) && lidar_msg->points[i].x > 0.7) {
      // https://github.com/Livox-SDK/Livox-SDK/wiki/Livox-SDK-Communication-Protocol
      // See [3.4 Tag Information]
      if ((lidar_msg->points[i].x > 2.0) &&
          (((lidar_msg->points[i].tag & 0x03) != 0x00) ||
           ((lidar_msg->points[i].tag & 0x0C) != 0x00))) {
...
bool LivoxFeatureExtraction::ParsePointCloudNoFeature(
  ...
  for (uint i = 1; i < plsize; i++) {
    if ((lidar_msg->points[i].line < n_scan) &&
        ((lidar_msg->points[i].tag & 0x30) == 0x10 ||
         (lidar_msg->points[i].tag & 0x30) == 0x00)) {
      valid_num++;
      if (valid_num % point_filter_num == 0) {
        ...
        if (((abs((*p_full_cloud)[i].x - (*p_full_cloud)[i - 1].x) > 1e-7) ||
             (abs((*p_full_cloud)[i].y - (*p_full_cloud)[i - 1].y) > 1e-7) ||
             (abs((*p_full_cloud)[i].z - (*p_full_cloud)[i - 1].z) > 1e-7)) &&
            ((*p_full_cloud)[i].x * (*p_full_cloud)[i].x +
                 (*p_full_cloud)[i].y * (*p_full_cloud)[i].y +
                 (*p_full_cloud)[i].z * (*p_full_cloud)[i].z >
             (blind * blind))) {
