...
void TrajectoryEstimator::AddUWBMeasurementAnalyticNURBS(
  ...
  for (const auto &dist_pair : uwb_measurement.anchor_distances) {
    auto anchor_it = uwb_measurement.anchor_positions.find(dist_pair.first);
    using Functor = analytic_derivative::UWBFactorNURBS;
    ceres::CostFunction *cost_function = new Functor(
        time_ns, dist_pair, *anchor_it, su, blending_matrix,
        cumulative_blending_matrix, S_GtoM, p_GinM, S_UtoI, p_UinI, uwb_weight);
