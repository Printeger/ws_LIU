...
int main(int argc, char **argv) {
  ...
  switch (lidar_type) {
    ...
    case OUST64:
      printf("OUST64\n");
      sub_points = n.subscribe("/os_cloud_node/points", 1000, oust64_handler,
                               ros::TransportHints().tcpNoDelay());
      break;
...
void horizon_handler(const livox_ros_driver2::CustomMsg::ConstPtr &msg) {
  ...
  for (int j = 0; j < N_SCANS; j++) {
    ...
    for (uint pt_idx = 0; pt_idx < plsize; pt_idx++) {
      types[pt_idx].range =
          pl[pt_idx].x * pl[pt_idx].x + pl[pt_idx].y * pl[pt_idx].y;
    ...
    if (0)  // For debug
    {
      if (pl_processed.size() % 1000 == 0) {
        printf(
            ...
            "\r\n",
            i, pl_orig.points[i].x, pl_orig.points[i].y, pl_orig.points[i].z,
            pl_processed.points.back().normal_x,
            pl_processed.points.back().normal_y,
            pl_processed.points.back().normal_z, yaw_angle,
            pl_processed.points.back().intensity);
        // printf("(%d, %.2f, %.2f)\r\n", pl_orig.points[i].ring,
        // pl_orig.points[i].t, pl_orig.points[i].range); printf("(%d, %d,
        // %d)\r\n", pl_orig.points[i].ring, 1, 2);
        cout << (int)(pl_orig.points[i].ring) << ", "
             << (pl_orig.points[i].t / 1e9) << ", " << pl_orig.points[i].range
             << endl;
      }
    }
  }
