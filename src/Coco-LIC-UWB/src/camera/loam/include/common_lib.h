...
// #define DEBUG_FILE_DIR(name) (std::string(std::string(ROOT_DIR) + "Log/" +
// name)) using vins_estimator = fast_lio;
...
template <typename T = double>
inline Eigen::Matrix<T, 3, 3> right_jacobian_of_rotion_matrix(
...
template <typename T = double>
Eigen::Matrix<T, 3, 3> inverse_right_jacobian_of_rotion_matrix(
    const Eigen::Matrix<T, 3, 1> &omega) {
  // Barfoot, Timothy D, State estimation for robotics. Page 232-237
  Eigen::Matrix<T, 3, 3> res_mat_33;
...
struct Camera_Lidar_queue {
  ...
  bool if_camera_can_process() {
    ...
    if (lidar_last_time <= cam_last_time) {
      ...
      // cout << "Camera can update, " << get_lidar_front_time() -
...
struct StatesGroup_r3live {
 public:
  ...
  StatesGroup_r3live operator+(
      const Eigen::Matrix<double, DIM_OF_STATES, 1> &state_add) {
    StatesGroup_r3live a = *this;
    // a.rot_end = this->rot_end * Sophus::SO3d::Exp_r3live(vec_3(state_add(0,
    // 0), state_add(1, 0), state_add(2, 0) ) );
    a.rot_end =
        this->rot_end * Exp_r3live(state_add(0), state_add(1), state_add(2));
...
#if ENABLE_CAMERA_OBS
    // Ext camera w.r.t. IMU
    a.rot_ext_i2c = this->rot_ext_i2c *
                    Exp_r3live(state_add(18), state_add(19), state_add(20));
  ...
  StatesGroup_r3live &operator+=(
      const Eigen::Matrix<double, DIM_OF_STATES, 1> &state_add) {
    this->rot_end = this->rot_end * Exp_r3live(state_add(0, 0), state_add(1, 0),
                                               state_add(2, 0));
...
#if ENABLE_CAMERA_OBS
    // Ext camera w.r.t. IMU
    this->rot_ext_i2c = this->rot_ext_i2c *
                        Exp_r3live(state_add(18), state_add(19), state_add(20));
  ...
  Eigen::Matrix<double, DIM_OF_STATES, 1> operator-(
      const StatesGroup_r3live &b) {
  ...
  static void display(const StatesGroup_r3live &state,
